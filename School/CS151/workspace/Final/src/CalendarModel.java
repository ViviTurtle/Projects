...
public class CalendarModel {
	 ...
	 *            the current day
	 */
	public CalendarModel(int month, int dayOfMonth, int year) {
	 ...
	 * 
	 * @param roomTypePrice
	 *            price of the room based on what type it is
	 * @param start
	 *            the beginning date
	 * @param end
	 *            the ending date
	 * @return ArrayList of rooms available between the beginning and ending
	 *         date
	 */
	public ArrayList<Room> availableRooms(int roomTypePrice, Day start, Day end) {
		...
		int index1 = search(0, reservations.size() - 1, sixtyBefore,
				sixtyBefore);
		int index2 = search(index1, reservations.size() - 1, sixtyAfter,
				sixtyAfter);
		...
		for (int i = index1; i < index2; i++) {
			...
			if (current.overlap(start, end)) {
				unavailable[current.getRoom().getRoomNumber()] = current
						.getRoom();
			}
		}
		...
		{
			for (int j = 10; j < 20; j++) {
				if (unavailable[j] == null) {
					available.add(hotel.getRoom(j));
				}
			}
		} else {
			for (int j = 0; j < 10; j++) {
		...
		/*
		 * for (int j = 0; j < 20; j++) { if (unavailable[j]==null) {
		 * available.add(hotel.getRoom(j)); } }
		 */
		return available;
	}
	 ...
	 * @param high
	 *            the high index of the range
	 * @param start
	 *            the start date to look for
	 * @param end
	 *            the end date to look for
	 ...
	 * 
	 * @param day
	 *            the day to check for reservations
	...
	/**
	 * Checks a date for reservations and returns in a string separated by new lines.
	 * 
	 * @param date
	 * the date to check for reservations
	 * @return s 
	 * the list of reservations on specified date
	 */
	public String listReservationsOn(Day date) {
		String s = "";
		ArrayList<Reservation> r = new ArrayList<Reservation>();
		try {
			viewAllReservation(date);
	 ...
	 * @param userID
	 *            the user who makes the reservation
	 * @return true if reservation was made, false if it wasn't
	 */
	public boolean makeReservation(Room room, Day dateStart, Day dateEnd,
			int userID) {
		// check if reservation is after current date
		// check if reservation is more than 60 days
		// check if room is available
		if (currentDay.compareTo(dateStart) < 0
				&& dateEnd.daysFrom(dateStart) < 60) {
	...
	public void cancelReservation(Reservation r) {
		...
		int index = search(0, reservations.size() - 1, r.getStartDate(),
				r.getEndDate());
	...
	class Hotel {
		...
		public Hotel() {
			...
			for (int i = 0; i < 10; i++) {
				Room l = new Room(i, LUXURIOUS_PRICE);// luxurious room #'s
														// range 0-9
				luxurious[i] = l;
				Room e = new Room(10 + i, ECONOMIC_PRICE);// economic room #'s
															// range 10-19
				economic[i] = e;
			}
		}
		 ...
		 * 
		 * @param roomNum
		 *            the number of the room to get
		 * @return the room with the provided room number
		 */
		public Room getRoom(int roomNum) {
			if (roomNum < 10) {
				return luxurious[roomNum];
			}
	...
	public String Reservation2Data(int index) {
		return reservations.get(index).toData();
	}
	public String reservationsToData() {
		String s = "";
		for(Reservation r:reservations){
			s+=r.toData();
		}
	...
	public boolean loadReservation(int aRoom, int aType, String aDateStart,
			String aDateEnd, int UserID) {
		// check if reservation is after current date
		// check if reservation is more than 60 days
		// check if room is available
		Room room = new Room(aRoom, aType);
		Day dateStart = Day.inputToDay(aDateStart);
		Day dateEnd = Day.inputToDay(aDateEnd);
		if (currentDay.compareTo(dateStart) < 0
				&& dateEnd.daysFrom(dateStart) < 60) {
			Reservation res = new Reservation(room, dateStart, dateEnd, UserID);
			if (!reservations.isEmpty()) {
