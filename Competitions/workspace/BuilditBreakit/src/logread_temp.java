...
public class logread_temp {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		while (!command.isEmpty()) {
			...
			case "-K":
				...
				try {
					...
					System.err.println("No authentication token supplied.");
					System.exit(-1);
				}
				break;
			case "-H":
			...
			case "-A":
				...
				try {
					...
					System.err
							.println("-A specified but missing lower and/or upper "
									+ "time bound flag specified.");
					System.exit(-1);
				} catch (NumberFormatException e) {
					System.err.println("-A specified but improper "
							+ "lower and/or upper time bound supplied.");
					System.exit(-1);
				}
				uFlag = true;
				break;
			case "-B":
				...
				try {
					...
					System.err
							.println("-B specified but missing lower and/or upper "
									+ "time bounds flag specified.");
					System.exit(-1);
				} catch (NumberFormatException e) {
					System.err.println("-B specified but improper "
							+ "lower and/or upper time bounds supplied.");
					System.exit(-1);
				}
				uFlag = true;
				break;
			case "-E":
				try {
					...
					System.err
							.println("-E specified but missing employee name.");
					System.exit(-1);
				}
				break;
			case "-G":
				try {
					...
					System.err.println("-G specified but missing guest name.");
					System.exit(-1);
				}
				break;
			default:
				if (path.isEmpty()) {
					...
					System.err
							.println("Multiple possible paths, unsupported flags, and/or "
									+ "missing flag tokens detected.");
					System.exit(-1);
				}
			}
			if (uFlag && uniqueFlag) {
				System.err
						.println("Only one of the following flags may be specified: "
								+ "-S -R -T -I -A -B");
				System.exit(-1);
			} else if (uFlag) {
				uniqueFlag = true;
		...
		if (!K || token.isEmpty()) {
			System.err.println("No authentication flag supplied.");
			System.exit(-1);
		}
		// TODO: Verify authentication token
		if (path.isEmpty()) {
			System.err.println("No path supplied.");
			System.exit(-1);
		}
		// TODO: Check path to make sure it's a valid path
		if (R && (employeeNames.isEmpty() && guestNames.isEmpty())) {
			System.err
					.println("If -R is specified, -E or -G must be specified.");
			System.exit(-1);
		}
		if (T && H) {
			System.err.println("Specifying -T and -H is not valid.");
			System.exit(-1);
		}
		// TODO: Prints
